...
Chuẩn hóa và kiểm chứng chức năng `Xác Thực Và Quản Lý Người Dùng` của ViFood-API theo flow AI4SE: bắt đầu từ Sys-docs và AGENTS, dùng Spec Kit artifacts, chọn Codex skills đúng phạm vi, thực hiện hoặc rà soát code Backend API, chạy pytest/contract checks, sau đó chờ người phát triển xác nhận và ghi evidence.
...
app/services/v1/auth_service.py
