# Tasks: Tích Hợp Lưu Ảnh S3

**Input**: Tài liệu thiết kế từ `/specs/005-storage-integration/`
...
- [ ] T001 Đọc docs storage/data ownership và backend-builder contract.
- [ ] T002 Rà soát env/config S3 không chứa secret hard-code.
- [ ] T003 Chọn skills: `vifood-implement-api-change`, `vifood-debug-extraction-flow`, `vifood-secret-security-audit`, `vifood-ai4se-evidence-log`.

## Phase 2: Foundation Review

**Goal**: Xác định router/service/repository/schema/test bị ảnh hưởng và ranh giới kiến trúc cần giữ.

- [ ] T004 Rà soát `S3Service` và mapping `image_ref`.
- [ ] T005 Rà soát thứ tự gọi S3 trong product service.
- [ ] T006 Rà soát error mapping khi upload/download URL lỗi.
...
- [ ] T007 [US1] Kiểm tra upload sau Builder success.
- [ ] T008 [US1] Kiểm tra Builder failure bỏ qua S3 upload.
- [ ] T009 [US2] Kiểm tra S3 failure không tạo success history.
- [ ] T010 [US2] Kiểm tra MongoDB không lưu ảnh base64 đầy đủ.
