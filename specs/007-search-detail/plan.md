...
Chuẩn hóa và kiểm chứng chức năng `Tìm Kiếm Và Chi Tiết Tri Thức` của ViFood-API theo flow AI4SE: bắt đầu từ Sys-docs và AGENTS, dùng Spec Kit artifacts, chọn Codex skills đúng phạm vi, thực hiện hoặc rà soát code Backend API, chạy pytest/contract checks, sau đó chờ người phát triển xác nhận và ghi evidence.
